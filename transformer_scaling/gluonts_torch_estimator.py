...
class PyTorchLightningEstimator(Estimator):
    ...
    def _train_model(
        ...
        **kwargs,
        ...
        training_network = self.create_lightning_module(path = ckpt_path)
        ...
        if ckpt_pars:
            checkpoint_callback = ModelCheckpoint(**ckpt_pars)
            callbacks = self.trainer_kwargs.get("callbacks", [])
        ...
        return checkpoint_callback, training_network, transformation, trainer
    ...
    def get_predictor(
        ...
        transformation = self.create_transformation()
        training_network = self.create_lightning_module(path)
        return self.create_predictor(transformation, training_network)
    ...
    def train_and_save(
        ...
        **kwargs,
        ...
        checkpoint_callback, _, _, _ = self._train_model(
            training_data,
            validation_data,
            from_predictor,
            num_workers,
    ...
    def train_model(
        ...
        **kwargs,
        ...
        checkpoint_callback, training_network, transformation, trainer = self._train_model(
            training_data,
            validation_data,
            from_predictor,
            num_workers,
        ...
        if checkpoint_callback:
            comm = MPI.COMM_WORLD
            comm.Barrier()
            best_model = self._create_lm_module(checkpoint_callback.best_model_path)
        else:
