...
from Transformers.DecoderModels import RNNDecoder
from Transformers.QuantileDecoder import QuantileDecoder
from Transformers.utils import _easy_mlp, _merge_series_time_dims, _split_series_time_dims, NormalizationIdentity, NormalizationStandardization, device
...
class TradingBot(nn.Module):
    ...
    def __init__(
        self,
        num_series: int,
        #optimization parameters for regularization
        gamma: float ,
        l_norm: int,
        #Encoder parameters
        ...
        if rnn_decoder is not None:
            self.decoder = RNNDecoder(input_dim=self.encoder.embedding_dim, **rnn_decoder)
        
        if quantile_decoder is not None:
            self.decoder = QuantileDecoder(input_dim=self.encoder.embedding_dim, **quantile_decoder)
